Shader "MyCustom/_Wobbletube"
{
    Properties
    {
        _FillAmounttube                 ("_FillAmounttube",                 Range(-1, 1))    = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // 1. 液体
                float liquidPart = step(i.fill, _FillAmounttube);
                float3 liquidColor = liquidPart * _LiquidColor.rgb;

                // 2. 液体和平面的交界处
                float foamPart = liquidPart - step(i.fill, _FillAmounttube - _LiquidFoamHeight);
